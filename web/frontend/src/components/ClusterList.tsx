...
interface Props {
  servers: Server[]
  onOpenDashboard: (cluster: Cluster) => void
}
...
export function ClusterList({ servers, onOpenDashboard }: Props) {
  ...
  const [dashConfirm, setDashConfirm] = useState<Cluster | null>(null)
        ...
        <div className="modal-overlay" onClick={() => setDashConfirm(null)}>
          <div className="modal" onClick={e => e.stopPropagation()} style={{ maxWidth: 360 }}>
            <div className="modal-header">
              <span>Open Dashboard — {dashConfirm.name}</span>
              <button className="modal-close" onClick={() => setDashConfirm(null)}>×</button>
            </div>
            <div className="modal-body" style={{ padding: '1.25rem 1.5rem' }}>
              <p style={{ margin: 0, color: 'var(--dim)', fontSize: '0.88rem' }}>
                {dashConfirm.members.length} nodes will be connected. Gateway routing follows the active profile.
              </p>
            </div>
